...
class Fun(commands.Cog, command_attrs=dict(hidden=False)):
    ...
    async def token(self, ctx: commands.Context[CodingBot]):
        ...
        embed = discord.Embed(
            title="Ha ha ha, I grabbed your bot token.",
    ...
    async def animal(
        self, ctx: commands.Context[CodingBot], animal: Optional[str] = None
    ):
        options = (
            "dog",
            "cat",
        ...
        if response.status in range(200, 300):
            ...
            embed = discord.Embed(
                title="Here's the animal image you asked.", color=discord.Color.random()
            )
            ...
            embed = discord.Embed(
                title="ERROR!",
                description=f"Received a bad status code of {response.status}",
            )
            embed.set_footer(
    ...
    async def binary_decode(self, ctx: commands.Context[CodingBot], binary: str):
        ...
        embed = discord.Embed(
            title="Encoded to binary", description=string, color=discord.Color.random()
        )
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
        )
    ...
    async def lyrics(self, ctx: commands.Context[CodingBot], *, query: str = None):
        if not query:
            embed = discord.Embed(
                title="Hey! I'm confused",
                description=f"You must provide a search argument or I couldn't find the lyrics",
            )
            embed.set_footer(
                text=f"Requested by {ctx.author}",
                icon_url=ctx.author.display_avatar.url,
    ...
    async def filter(self, ctx: commands.Context[CodingBot]):
        embed = discord.Embed(
            ...
            description="Available methods: `invert`, `greyscale`, `colour [hex]`",
            color=discord.Color.random(),
        )
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
    ...
    async def filter_invert(
        self, ctx: commands.Context[CodingBot], member: discord.Member = None
    ):
        if not member:
            member = ctx.author
        ...
        embed = discord.Embed(
            title="Filter command - Invert", color=discord.Color.random()
        )
    ...
    async def filter_greyscale(
        ...
        embed = discord.Embed(
            title="Filter command - Greyscale", color=discord.Color.random()
        )
        embed.set_image(url=response)
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
    ...
    async def filter_colour(
        ...
        if not hex_code:
            embed = discord.Embed(
                title="ERROR!", description="No Hex? Hex colour code is required"
            )
            embed.set_footer(
                text=f"Requested by {ctx.author}",
                icon_url=ctx.author.display_avatar.url,
        ...
        pfp = member.display_avatar.url
        response = await self.http.api["some-random-api"]["filters"]["greyscale"](
            pfp, hex_code
        )

        embed = discord.Embed(
            title="Filter command - Colour", color=discord.Color.random()
        )
        embed.set_image(url=response)
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
    ...
    async def filter_brightness(
        ...
        embed = discord.Embed(
            title="Filter command - Brightness", color=discord.Color.random()
        )
        embed.set_image(url=response)
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
    ...
    async def filter_threshold(
        ...
        embed = discord.Embed(
            title="Filter command - Threshold", color=discord.Color.random()
        )
        embed.set_image(url=response)
        embed.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.display_avatar.url
