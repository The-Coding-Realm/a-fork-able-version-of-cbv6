...
class Moderation(commands.Cog, command_attrs=dict(hidden=False)):
    ...
    async def log(
        ...
        **kwargs: Any,
        ...
        definition: Dict[str, Any] = {
            ...
            "warn": {
                "action": "warned",
                "undo_action": f"removed warning ({kwargs.get('warn')}) from"
                if kwargs.get("warn")
                else "removed all warnings from",
                "icon": ":warning:",
                "undo_icon": ":flag_white:",
                "color": discord.Color.yellow(),
            },
        }
        action_info = definition.get(action, ValueError(f"Invalid action: {action}"))
    ...
    async def kick(
        ...
        *,
        ...
        try:
            await member.send(
                "You have been :boot: **Kicked** :boot: from "
                f"**{ctx.guild.name}**. \nReason: {reason}"
            )
        ...
        *,
        ...
        try:
            ...
            return await self.bot.reply(
                ctx, f"{user.mention} is not banned from this server."
            )
        else:
    ...
    async def mute(
        ...
        *,
        ...
        try:
            await member.send(
                "You have been :mute: **Muted** :hammer: from "
                f"**{ctx.guild.name}**. \nReason: {reason}"
            )
        except discord.Forbidden:
            pass
    ...
    async def massban(
        ...
        if len(users) > 100:
            return await self.bot.reply(
                ctx, "Please provide less than 100 users to ban."
            )
    ...
    async def verify_member(
        self, ctx: commands.Context[CodingBot], target: discord.Member
    ):
        """
        ...
        if member in target.roles:
            ...
            embed = discord.Embed(
                ...
                description=f"{target.mention} was successfully verified",
            )
            embed.set_footer(
                text=f"Command executed by {ctx.author}",
                icon_url=ctx.author.display_avatar.url,
    ...
    async def whois(
        self, ctx: commands.Context[CodingBot], member: Optional[discord.Member] = None
    ) -> None:
        """
        ...
        target: discord.Member = member or ctx.author  # type: ignore
        embed = discord.Embed(
            title=f"Showing user info : {member}", color=discord.Color.random()
        )
        embed.set_thumbnail(url=target.display_avatar.url)
        # Support for nitro users
        created_at_string = target.created_at.strftime("%d %B, %Y")
        created_ago = humanize.precisedelta(
            (discord.utils.utcnow() - target.created_at).total_seconds(),
        ...
        embed.add_field(
            ...
            value=f"{created_at_string} ({created_ago} ago)",
            inline=False,
        )
        embed.add_field(name="Discord Joined at", value=target.joined_at, inline=False)
    ...
    async def delete(
        ...
        await self.bot.reply(
            ctx, f"{message.content} ||({message.id} was deleted by {ctx.author})||"
        )
    ...
    @commands.has_permissions(manage_messages=True)
    async def welcomer(self, ctx: commands.Context[CodingBot]) -> None:
        """
        Welcomer commands
